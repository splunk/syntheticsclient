...
func TestDeleteBrowserCheckV2ReturnsErrorOnNon2xxStatus(t *testing.T) {
	setup()
	defer teardown()

	testMux.HandleFunc("/v2/tests/browser/20", func(w http.ResponseWriter, r *http.Request) {
		testMethod(t, r, "DELETE")
		w.WriteHeader(http.StatusNotFound)
	})

	status, err := testClient.DeleteBrowserCheckV2(20)
...
func TestDeleteBrowserCheckV2ReturnsErrorWhenRequestFails(t *testing.T) {
	unreachableClient := NewConfigurableClient("apiKey", "realm", ClientArgs{publicBaseUrl: "http://127.0.0.1:1"})

	status, err := unreachableClient.DeleteBrowserCheckV2(21)
